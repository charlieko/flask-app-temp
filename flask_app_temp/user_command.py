...
from flask_app_temp.models.user import User
from flask_app_temp import db
...
def create_admin(name, email, username, password):
  if not name or not email or not username or not password:
    print "Please pass all required parameters.\n Usage: python -m flask_app_temp user create_admin -n <name> -e <email> -u <username> -p <password>"
  else:
...
def reset_password(username, password):
  if not username or not password:
    print "Please pass all required parameters.\n Usage: python -m flask_app_temp user reset_password -u <username> -p <password>"
  else:
...
def set_admin_level(username, admin_level):
  if not username or not admin_level:
    print "Please pass all required parameters.\n Usage: python -m flask_app_temp user set_admin_level -u <username> -a <admin_level>"
  else:
    user = User.query.filter_by(username=username).first()
    if not user:
      print "No such user found"
